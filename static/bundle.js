...
var author$project$Main$api = function (query) {
	return 'http://www.omdbapi.com/?apikey=8eff793c' + ('&s=' + query);
};
var author$project$Main$Movie = F2(
	function (title, image) {
		return {aa: image, at: title};
	});
...
var author$project$Main$decodeJson = A2(
...
var elm$http$Http$request = function (r) {
	return elm$http$Http$command(
		elm$http$Http$Request(
			{v: false, ax: r.ax, aA: r.aA, e: r.e, h: r.h, i: r.i, j: r.j, aQ: r.aQ}));
};
var elm$http$Http$get = function (r) {
	return elm$http$Http$request(
		{ax: elm$http$Http$emptyBody, aA: r.aA, e: _List_Nil, h: 'GET', i: elm$core$Maybe$Nothing, j: elm$core$Maybe$Nothing, aQ: r.aQ});
};
var author$project$Main$getMovies = function (query) {
	return elm$http$Http$get(
		{
			aA: A2(elm$http$Http$expectJson, author$project$Main$GotResults, author$project$Main$decodeJson),
			aQ: author$project$Main$api(query)
		});
};
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 1:
				return _Utils_Tuple2(
					model,
					author$project$Main$getMovies(model.H));
			default:
				var response = msg.a;
				if (!response.$) {
					var results = response.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{L: results}),
...
var author$project$Main$Search = {$: 1};
...
var author$project$Main$movieItem = function (movie) {
	return A2(
		elm$html$Html$li,
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('movie-item')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$src(movie.aa)
					]),
				_List_Nil),
				A2(
				elm$html$Html$span,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$Attributes$placeholder('Search for a movie...'),
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('movies')
					]),
				A2(elm$core$List$map, author$project$Main$movieItem, model.L))
			]));
};
...
var author$project$Main$main = elm$browser$Browser$element(
	{aF: author$project$Main$init, aO: author$project$Main$subscriptions, aP: author$project$Main$update, aR: author$project$Main$view});
